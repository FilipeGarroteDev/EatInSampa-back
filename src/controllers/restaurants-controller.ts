...
async function insertRestaurant(req: Request, res: Response) {
	...
	try {
		...
		await insertNewRestaurant({name, categoryId: Number(categoryId), creatorId: userId} as NewRestaurant);
...
async function deleteRestaurant(req: Request, res: Response) {
	...
	try {
		const hasRestaurant: Restaurant = await searchRestaurantById(
			id
		);
